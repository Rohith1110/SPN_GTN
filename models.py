...
from graph_transformer_edge_layer_pyg import GraphTransformerEdgeLayer
from dataset import compute_laplacian_positional_encoding
...
class GNNLayer(torch.nn.Module):
    ...
    def get_conv(self, in_channels: int, out_channels: int, heads: int, args: Namespace, final_layer: bool, layer: int):
        ...
        # GATv2Conv integration entry
        elif args.GNN_model == 'GATv2Conv':
            if not final_layer:
                assert out_channels % heads == 0, f'Out channels {out_channels} must be divisible by heads {heads} for GATv2Conv.'
                return GATv2Conv(in_channels, out_channels // heads, heads, concat=True, dropout=args.dropout_prob)
            else:
                return GATv2Conv(in_channels, out_channels, heads, concat=False, dropout=args.dropout_prob)
...
class MultiGNNLayers(torch.nn.Module):
    def __init__(self, args: Namespace, num_features: int):
        ...
        #     in_channels = num_features + args.pe_dim
        # else:
        ...
        for i, size in enumerate(args.GNN_hidden_sizes):
            if args.GNN_model == 'GATv2Conv':
                heads = args.GATv2_heads[i]
            elif args.GNN_model == 'GATConv':
                heads = args.GAT_heads[i]
            else:
                heads = None
        ...
        if args.use_gt_layer and args.num_transformer_layers > 0:
            ...
            in_dim = args.GNN_hidden_sizes[-1] if args.use_laplacian_pe else in_channels
...
class NodeClassifier(torch.nn.Module):
    def __init__(self, args: Namespace, in_channels: int, num_classes: int = 2):
        ...
        if args.skip_connections == 'identity':
            ...
            if args.GNN_model == 'GATv2Conv':
                heads = args.GATv2_heads[-1]
            elif args.GNN_model == 'GATConv':
                heads = args.GAT_heads[-1]
            else:
                heads = None
...
class JointModel(torch.nn.Module):
    def __init__(self, args: Namespace, num_features: int, num_classes: int = 2):
        ...
        if args.GNN_model == 'GATConv': 
            ...
            assert len(args.GNN_hidden_sizes) == len(args.GATv2_heads) - 1
        model = GraphUNetWithDropout if args.GNN_model == 'GraphUNet' else MultiGNNLayers
        self.multi_gnn_layers = model(args, num_features)
    ...
    def combine_weights(self, node_weights: OrderedDict, edge_weights: OrderedDict, device: torch.device):
        if self.args.separate_model_after_proxy:
