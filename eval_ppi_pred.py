...
def get_latest_pt_file(file_list):
    if not file_list:
        return None
    # Extract timestamp from filename and use it for sorting
    def get_timestamp(filepath):
        # Assuming filename format includes "_MM-DD_HH-MM-SS"
        try:
            timestamp_str = str(filepath).split('_')[-2] + '_' + str(filepath).split('_')[-1].replace('.pt','')
            return datetime.strptime(timestamp_str, '%m-%d_%H-%M-%S')
        except:
            return datetime.min  # Return minimum date if parsing fails
    
    return max(file_list, key=get_timestamp)
